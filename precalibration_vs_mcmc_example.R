...
pdf(file="precal_vs_mcmc_theta1.pdf",10,8)  

plot(1:NI,chain1[,1],type="l",main="theta1",xlab="iterations",
     ylab="value",col = "red",ylim=c(-15,-3),lwd = 3)
...
pdf(file="precal_vs_mcmc_theta2.pdf",10,8)

plot(1:NI,chain2[,1],type="l",main="theta2",xlab="iterations",
     ylab="value",col = "red",ylim=c(-5,15),lwd = 3)
#lines(c(nSamples,test_ss),e_theta1_chain,type="l",col = "blue")
...
pdf(file="overlaid_densities_mcmc_vs_precal.pdf",8,10)  
par( mfrow= c(2,1))
par(fig=c(0,1,0.5,1),mar=c(4,4,4,4))

plot(density(chain1[,1]),main="",xlab ="theta1",xlim=c(-12,-9),
     ylim=c(0,1.5),col = "purple",lwd=3)
lines(density(all_good_theta1[which(all_good_ss==NI)]),col = "turquoise",lwd=3)
abline(v = True_theta1, lwd = 3,lty = 1,col="black")
legend("topright", c("mcmc","precalibration","truth"),
       lty=1, lwd = 3, col = c("purple","turquoise","black"))

plot(density(chain2[,1]),main="",xlab ="theta2",xlim=c(-3,6),
     ylim=c(0,0.5),col = "purple",lwd=3)
lines(density(all_good_theta2[which(all_good_ss==NI)]),col = "turquoise",lwd=3)
